            ...
            for env_name in config.envs.keys():
                ...
                for episode_id in range(config.max_episode_eval):
                    ...
                    while round_done is False:
                        ...
                        if round_id % 5 == 0:
                            if old_stats["gns"] is None:
                                action["num_rollout_workers"] = config.num_rollout_workers_min
                            else:
                                if gns > old_stats["gns"]:
                                    action["num_rollout_workers"] = np.clip(
                                        int(action["num_rollout_workers"] * (gns / old_stats["gns"])), 
                                        a_min=config.num_rollout_workers_min,
                                        a_max=config.num_rollout_workers_max
                                    )
                        old_stats["gns"] = gns
