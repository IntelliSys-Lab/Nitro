        ...
        for algo_name in config.algos:
            for env_name in config.envs.keys():
                ...
                for episode_id in range(config.max_episode_eval):
                    ...
                    reward_window = collections.deque(maxlen=config.window_size)
                    reward_window_grad = 0
                    ...
                    while round_done is False:
                        ...
                        if len(reward_window) > 1:
                            reward_window_grad = np.abs(np.mean(np.gradient(reward_window)))
                            if reward_window_grad <= config.window_grad_clip:
                                action["num_rollout_workers"] = np.clip(
                                    action["num_rollout_workers"] - config.window_unit,
                        ...
                        print("reward_window_grad: {}".format(reward_window_grad))
                        print("---")
                        print("PAC related:")
                        print("m: {}".format(m))
                        try:
                        ...
                        reward_window.append(info["eval_reward"])
